...
%                     pt(k, (zz-1)*d^2+(rr-1)*d+cc)=nnim(s*floor(mod(jj,(B*A))/B)+floor(mod(kk ,Ds2)/Ds)+rr , s*mod(jj,B)+mod(kk,Ds)+cc , s*floor(jj/(B*A))+floor(kk/Ds2)+zz );
...
%         temp3 = sum(Ve<=x(2,i)*medlmd); medlmdt = median(Ve((D3-temp3+1):end ));
%         Tau = x(1,i)* (medlmdt);
%         %sigma=beta* (medlmdt);
%         %Tau = tau*sigma;
...
%                         sm(temp2(1) , temp2(2) , temp2(3))= sm(temp2(1) , temp2(2) , temp2(3))+ (XFT(iii , (zz-1)*D2+(rr-1)*d+cc));
...
% %dnim = sm./COUNT;%biased estimation
        dnim = truncateslice(dnim, 3);
        index = find(im>0);
        ma = max(im,[],'all');
%         psnr(i) = -20*log10(255/sqrt(mean((im(index)-dnim(index)).^2)));
        psnr = -20*log10(ma/sqrt(mean((im(index)-dnim(index)).^2)));
        psnrssim(i) = psnr;
    %end
end
end
