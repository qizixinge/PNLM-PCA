function [dnim, noise_mapr] = NLPCAmy(nnim,tau,beta, T)
%developed method from Manjon 2015
d=3;M=27;w=3;s=1;
xw=2*w+1; W=xw^3;
% XW=zeros(1,xw);
% for i=1:xw
...
nim=padarray(nnim , [ S(s-mod(size(nnim , 1)-d ,s)) ,S( s-mod(size(nnim , 2)-d ,s)) ,S(s-mod(size(nnim , 3)-d,s))  ] , 'symmetric' , 'post');
%nim=nnim;
knim=padarray(nim,[(xw-d)/2 (xw-d)/2 (xw-d)/2],'symmetric');
%numofwindow=(size(nnim, 1)/xw)*(size(nnim, 2)/xw)*(size(nnim, 3)/xw);
a=size(nim, 1); b=size(nim, 2); c=size(nim, 3);
aa=size(knim,1);bb=size(knim,2);cc=size(knim,3);
%A=size(knim, 1)-xw+1; B=size(knim, 2)-xw+1; C=size(knim, 3)-xw+1;
A=(a-d)/s+1;B=(b-d)/s+1;C=(c-d)/s+1;
...
sm = zeros(aa, bb, cc  ); smg = zeros( aa, bb, cc ); COUNT = zeros( aa, bb, cc );
for j=1:numofwindow
    jj = j-1;
    for cc=1:d
        for rr=1:d
    ...
    %for k=1:numofpatchw
        for r=1:numofpatchw
            dist2(r) = sum((pt((numofpatchw+1)/2 , : )-pt(r , : )).^2);
        end
        [~ , distsx] = mink( dist2 ,M);
        for rr=1:M
            XTO(rr , :) = pt(distsx(rr) , : );
...
dnim = kdnim( 3:(size(nnim,1)+2) , 3:(size(nnim,2)+2) , 3:(size(nnim,3)+2));
noise_mapr=knoise_mapr( 3:(size(nnim,1)+2) , 3:(size(nnim,2)+2) , 3:(size(nnim,3)+2));
